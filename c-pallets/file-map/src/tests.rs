...
fn registration_scheduler_should_work() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_err!(
			FileMap::registration_scheduler(RuntimeOrigin::signed(2), 1, ip.clone()),
			Error::<Test>::AlreadyRegistration
		);
		assert_err!(
			FileMap::registration_scheduler(RuntimeOrigin::signed(2), 3, ip),
