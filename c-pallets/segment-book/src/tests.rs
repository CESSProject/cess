...
fn vpa_vpb_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(SegmentBook::intent_submit(Origin::signed(1),1,1,4,vec![vec![5]],vec![6],vec![7]));	
		...
		assert_ok!(SegmentBook::intent_submit_po_st(Origin::signed(1),1,1,1));	
		assert_ok!(SegmentBook::submit_to_vpb(Origin::signed(1),1,1,vec![4],vec![5]));	
		assert_ok!(SegmentBook::verify_in_vpb(Origin::signed(1),1,1,true));	
	});
}
...
fn vpc_vpd_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(SegmentBook::intent_submit_po_st(Origin::signed(1),1,1,2));	
		assert_ok!(SegmentBook::submit_to_vpd(Origin::signed(1),1,1,vec![vec![4]],vec![vec![5]]));	
		assert_ok!(SegmentBook::verify_in_vpd(Origin::signed(1),1,1,true));	
	});
}