...
use cp_cess_common::{DataType, Hash, IpAddress};
use frame_benchmarking::account;
use frame_support::{assert_noop, assert_ok, bounded_vec};
use pallet_file_bank::{FillerInfo, SliceInfo, UserBrief};
...
// fn to_bounded_vec2<T: Clone>(vv: Vec<Vec<T>>) -> BoundedVec<BoundedVec<T, StringLimit>,
// StringLimit> {     let mut r: BoundedVec<BoundedVec<T, StringLimit>, StringLimit> =
// Vec::new().try_into().unwrap();     // let mut t = &r;
//     // vv.iter().map(|v| {
...
fn submit_challenge_prove_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			SegmentBook::submit_challenge_prove(
				RuntimeOrigin::signed(miner_acc.clone()),
				prove_list.clone()
			),
			Error::<Test>::NoChallenge
		);
		...
		assert_ok!(SegmentBook::submit_challenge_prove(
			RuntimeOrigin::signed(miner_acc.clone()),
			prove_list
		));
		...
		let event = Sys::events()
			.pop()
			.expect("Expected at least one ChallengeProof to be found")
			.event;
		assert_eq!(
			mock::RuntimeEvent::from(Event::ChallengeProof { miner: miner_acc, file_id }),
			event
		);
	});
}
...
fn verify_proof_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(SegmentBook::verify_proof(
			RuntimeOrigin::signed(controller1.clone()),
			verify_result_list.clone()
		));
		assert_eq!(0, UnVerifyProof::<Test>::try_get(controller1.clone()).unwrap().len());

		let event = Sys::events()
			.pop()
			.expect("Expected at least one VerifyProof to be found")
			.event;
		assert_eq!(
			mock::RuntimeEvent::from(Event::VerifyProof { miner: miner_acc, file_id }),
			event
		);
	});
}
...
fn verify_proof_on_punish() {
	new_test_ext().execute_with(|| {
		...
		//FIXME! the punish action is hard to test now, as it's depend concrete pallet: sminer.
		// Suggest doing this use Trait instead.
		assert_ok!(SegmentBook::verify_proof(
			RuntimeOrigin::signed(controller1.clone()),
			verify_result_list
		)); // the last parameter indicate whether punish
		assert_eq!(0, UnVerifyProof::<Test>::try_get(controller1.clone()).unwrap().len());
		...
		let event = Sys::events()
			...
			.event;
		assert_eq!(
			mock::RuntimeEvent::from(Event::VerifyProof {
				miner: miner_acc.clone(),
				file_id: mfi.file_hash
			}),
			event
		);
