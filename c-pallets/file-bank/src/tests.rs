...
fn upload_declaration_alias(
	...
	FileBank::upload_declaration(RuntimeOrigin::signed(account.clone()), file_hash, user_brief)
}

fn upload_file_alias(
	_account: AccountId,
	controller: AccountId,
	file_info: &MockingFileBankInfo,
) -> DispatchResult {
	let MockingFileBankInfo { file_hash, file_size, slice_info } = file_info.clone();
	FileBank::upload(RuntimeOrigin::signed(controller), file_hash, file_size, slice_info)
}
...
fn add_power_for_miner(controller: AccountId, miner: AccountId) -> DispatchResult {
	...
	for i in 0..max {
		...
		filler_list.push(FillerInfo::<Test> {
			index: 1,
...
fn buy_space_works() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			mock::RuntimeEvent::from(Event::BuySpace {
				acc: acc1,
				storage_capacity: space_gb * 1024 * 1_048_576,
				spend: price
			}),
			event
		);
...
fn expansion_space_work() {
	new_test_ext().execute_with(|| {
		...
		let event = Sys::events().pop().expect("Expected at least one BuySpace to be found").event;
		assert_eq!(
			mock::RuntimeEvent::from(Event::ExpansionSpace {
				acc: acc1,
				expansion_space: 1024 * 1024 * 1024,
				fee: price,
			}),
			event
		);
...
fn renewal_space_work() {
	new_test_ext().execute_with(|| {
		...
		let event = Sys::events().pop().expect("Expected at least one BuySpace to be found").event;
		assert_eq!(
			mock::RuntimeEvent::from(Event::RenewalSpace { acc: acc1, renewal_days, fee: price }),
			event
		);
...
fn upload_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(create_new_bucket(acc1.clone(), bucket_name.clone()));
		assert_ok!(upload_declaration_alias(
			acc1,
			file_name.clone(),
			mfi.file_hash.clone(),
			bucket_name.clone()
		...
		assert_noop!(
			upload_file_alias(acc1, controller1, &mfi),
			Error::<Test>::ScheduleNonExistent
		);
		...
		let file_slice_info = UserFileSliceInfo { file_hash: file_hash.clone(), file_size };
		...
		let event =
			Sys::events().pop().expect("Expected at least one FileUpload to be found").event;
...
fn upload_should_not_work_when_insufficient_storage() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(upload_declaration_alias(
			acc1,
			"cess-book".as_bytes().to_vec(),
			mfi.file_hash,
			bucket_name.clone()
		));
		assert_noop!(
			upload_file_alias(acc1.clone(), controller1.clone(), &mfi),
...
fn delete_file_works() {
	new_test_ext().execute_with(|| {
		...
		let bucket_name = "cess-bucket".as_bytes().to_vec();
		assert_noop!(
			FileBank::delete_file(
				RuntimeOrigin::signed(acc1.clone()),
				acc1.clone(),
				mfi.file_hash.clone()
			),
		...
		assert_noop!(
			FileBank::delete_file(
				RuntimeOrigin::signed(mock::account2()),
				mock::account2(),
				mfi.file_hash.clone()
			),
			Error::<Test>::NotOwner
		...
		assert_ok!(FileBank::delete_file(
			RuntimeOrigin::signed(acc1.clone()),
			acc1.clone(),
			mfi.file_hash.clone()
		));
		...
		let event =
			Sys::events().pop().expect("Expected at least one DeleteFile to be found").event;
		assert_eq!(
			mock::RuntimeEvent::from(Event::DeleteFile {
				operator: acc1.clone(),
				owner: acc1.clone(),
				file_hash: mfi.file_hash
			}),
			event
		);
	});
...
fn upload_filler_work() {
	new_test_ext().execute_with(|| {
		...
		let controller1 = mock::controller1();
		assert_noop!(
			add_power_for_miner(controller1.clone(), miner1.clone()),
			Error::<Test>::ScheduleNonExistent
		);
		assert_ok!(register_scheduler(stash1.clone(), controller1.clone()));
		assert_noop!(
			add_power_for_miner(controller1.clone(), miner1.clone()),
			pallet_sminer::Error::<Test>::NotMiner
		);
		assert_ok!(register_miner(miner1));
...
fn clear_invalid_file_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(FileBank::clear_invalid_file(
			RuntimeOrigin::signed(miner1.clone()),
			file_hash_list[0]
		));
...
fn create_bucket_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(FileBank::create_bucket(
			RuntimeOrigin::signed(acc1.clone()),
			acc1.clone(),
			bound_bucket_name.clone()
		));
...
fn delete_bucket_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(FileBank::delete_bucket(
			RuntimeOrigin::signed(acc1.clone()),
			acc1.clone(),
			bound_bucket_name.clone()
		));
...
fn transfer_ownership_works() {
	new_test_ext().execute_with(|| {
		...
		let mfi = MockingFileBankInfo::default();
		let bucket_name = "cess-bucket".as_bytes().to_vec();
		let bound_bucket_name: BoundedVec<u8, NameStrLimit> =
		...
		assert_ok!(FileBank::ownership_transfer(
			RuntimeOrigin::signed(acc1.clone()),
			bound_bucket_name,
			target_brief,
			mfi.file_hash.clone()
...
fn transfer_ownership_exception() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			FileBank::ownership_transfer(
				RuntimeOrigin::signed(acc2.clone()),
				bound_bucket_name.clone(),
				target_brief.clone(),
				mfi.file_hash.clone()
			),
