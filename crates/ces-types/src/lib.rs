...
pub mod messaging {
	...
	pub enum WorkerEvent {
		...
		///  When a worker start to compute, push this message to the worker to start the benchmark
		/// task.   session_id: Generated by pallet. Each computing session should have a unique
		/// session_id.
		Started { session_id: u32, init_v: U64F64Bits, init_p: u32 },
		/// pallet-computing --> worker
		///  When a worker entered CoolingDown state, push this message to the worker, so that it
		/// can stop the  benchmark task.
		Stopped,
		/// pallet-computing --> worker
		///  When a worker entered Unresponsive state, push this message to the worker to suppress
		/// the subsequent  heartbeat responses.
		EnterUnresponsive,
		/// pallet-computing --> worker
		///  When a worker recovered to WorkerIdle state from Unresponsive, push this message to
		/// the worker to  resume the subsequent heartbeat responses.
		ExitUnresponsive,
	}
	...
	bind_topic!(WorkingReportEvent, b"cess/mining/report");
	...
	bind_topic!(WorkingInfoUpdateEvent<BlockNumber>, b"^cess/mining/update");
	...
	pub struct WorkingInfoUpdateEvent<BlockNumber> {
		...
		/// Workers that do not responce the heartbeat challenge in time. Each delay only report
		/// once.
	...
	bind_topic!(GatekeeperLaunch, b"cess/gatekeeper/launch");
	#[derive(Encode, Decode, Clone, Debug, PartialEq, Eq, TypeInfo)]
	...
	impl GatekeeperLaunch {
		pub fn first_gatekeeper(
			...
			GatekeeperLaunch::FirstGatekeeper(NewGatekeeperEvent { pubkey, ecdh_pubkey })
		}
		...
		pub fn rotate_master_key(
			...
			GatekeeperLaunch::RotateMasterKey(RotateMasterKeyEvent { rotation_id, gk_identities })
		}
	...
	bind_topic!(GatekeeperChange, b"cess/gatekeeper/change");
	#[derive(Encode, Decode, Clone, Debug, PartialEq, Eq, TypeInfo)]
	...
	impl GatekeeperChange {
		pub fn gatekeeper_registered(
			...
			GatekeeperChange::Registered(NewGatekeeperEvent { pubkey, ecdh_pubkey })
		}
	...
	bind_topic!(KeyDistribution<BlockNumber>, b"cess/gatekeeper/key");
	#[derive(Encode, Decode, Clone, Debug, PartialEq, Eq, TypeInfo)]
	pub enum KeyDistribution<BlockNumber> {
		...
		///
		/// The origin cannot be Gatekeeper, else the leakage of old master key will further leak
		/// the following keys
	...
	// Messages: Gatekeeper
	bind_topic!(GatekeeperEvent, b"cess/gatekeeper/event");
	#[derive(Encode, Decode, Clone, Debug, PartialEq, Eq, TypeInfo)]
...
pub use sp_core::sr25519::{
	Public as WorkerPublicKey, Public as MasterPublicKey, Public as EcdhPublicKey,
