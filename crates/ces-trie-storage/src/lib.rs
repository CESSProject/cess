...
pub fn load_trie_backend<H: Hasher, A, B>(
	...
	B: AsRef<[u8]>,
{
	...
	let mut mdb = Default::default();
	{
		...
		// pairs.sort_by(|l, r| l.0.as_ref().cmp(r.0.as_ref()));
	...
	De: Deserializer<'de>,
{
	...
	let mdb = MemoryDB::from_inner(
		kvs.into_iter().map(|(data, rc)| (H::hash(data.as_ref()), (data, rc))).collect(),
	);
	...
	H::Out: Codec + Ord,
{
	...
	/// Calculate the new state root given storage changes. Returns the new root and a transaction
	/// to apply.
	...
	pub fn calc_root_if_changes<'a>(
		...
		self.0.full_storage_root(
			delta.iter().map(|(k, v)| (k.as_ref(), v.as_ref().map(|v| v.as_ref()))),
			child_deltas.iter().map(|(k, v)| {
				(k, v.iter().map(|(k, v)| (k.as_ref(), v.as_ref().map(|v| v.as_ref()))))
			}),
