use ces_trie_storage::*;
use serde::{Deserialize, Serialize};
use sp_core::Hasher;
use sp_runtime::{traits::Hash, StateVersion};
use sp_trie::{LayoutV0 as Layout, TrieConfiguration as _};
...
fn load_genesis_trie(spec_filename: Option<&str>) -> TrieStorage<NativeBlakeTwo256> {
	let mut trie: TrieStorage<NativeBlakeTwo256> = Default::default();
	let json_str =
		std::fs::read_to_string(data_dir().join(spec_filename.unwrap_or("dev-spec.json"))).unwrap();
...
fn map_storage_collection(collection: TestStorageCollection) -> StorageCollection {
	collection.into_iter().map(|(k, v)| (k.0, v.map(|v| v.0))).collect()
}

#[test]
fn test_genesis_root() {
	let trie = load_genesis_trie(None);
...
fn test_apply_main_changes() {
	let mut trie = load_genesis_trie(None);
