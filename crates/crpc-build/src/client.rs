...
pub fn generate<T: Service>(
    ...
    quote! {
        ...
        pub mod #client_mod {
            ...
            where
                Client: crpc::client::RequestClient
            {
...
fn generate_unary<T: Method>(
    ...
    quote! {
        pub async fn #ident(
            &self,
            request: #request,
        ) -> Result<#response, crpc::client::Error> {
            let response = self.client.request(#path, crpc::codec::encode_message_to_vec(&request)).await?;
            Ok(crpc::Message::decode(&response[..])?)
        }
    }
}
