...
fn decode_auth_data(ver: u16, input: &mut &[u8]) -> Result<AuthData, parity_scale_codec::Error> {
    match ver {
        3 => {
        ...
        _ => Err(parity_scale_codec::Error::from("unsupported quote version")),
    }
}
    ...
    fn decode<I: Input>(input: &mut I) -> Result<Self, parity_scale_codec::Error> {
        ...
        if header.version > 4 {
            ...
            if body.body_type != BODY_SGX_ENCLAVE_REPORT_TYPE {
                return Err(parity_scale_codec::Error::from("unsupported body type"));
            }
...
impl Quote {
    ...
    pub fn fmspc(&self) -> Result<Fmspc, Error> {
        ...
        let certs = super::utils::extract_certs(raw_cert_chain)?;
        let extension_section = super::utils::get_intel_extension(&certs[0])?;
        super::utils::get_fmspc(&extension_section)
    }
}
