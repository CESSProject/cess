...
impl fmt::Debug for CesealMasterKey {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        use ces_crypto::sr25519::Persistence;
        use rsa::pkcs8::EncodePrivateKey;
        f.debug_struct("CesealMasterKey")
            .field("rsa_priv_key", &self.rsa_priv_key.to_pkcs8_der().map_err(|_| fmt::Error)?)
            .field("sr25519_keypair", &self.sr25519_keypair.dump_secret_key())
            .finish()
    }
}
    ...
    pub fn unseal(sealing_path: &str, identity_key: &sr25519::Pair, sealer: &impl Sealing) -> Result<RsaDer, Error> {
        ...
        let secret = match versioned_data {
            MasterKeySeal::V1(data) => {
                if !identity_key.verify_data(&data.signature, &data.secret.encode()) {
...
pub use persistence::is_master_key_exists_on_local;
