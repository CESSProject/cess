            ...
            let (code, data) = match result {
                ...
                Err(err) => {
                    ...
                    if json {
                        ...
                        (code, crpc::codec::encode_message_to_vec(&err))
                    }
                }
            };
            (code, data)
...
impl<Platform: pal::Platform> RpcService<Platform> {
    pub fn lock_ceseal(
        ...
        allow_rcu: bool,
        allow_safemode: bool,
    ) -> RpcResult<LogOnDrop<MutexGuard<'_, Ceseal<Platform>>>> {
        debug!(target: "cestory::lock", "Locking cestory...");
        let guard = self.cestory.lock().unwrap();
...
impl<Platform: pal::Platform + Serialize + DeserializeOwned> CesealApi for RpcService<Platform> {
    ...
    async fn get_info(&mut self, _request: ()) -> RpcResult<pb::CesealInfo> {
    ...
    async fn dispatch_blocks(&mut self, request: pb::Blocks) -> RpcResult<pb::SyncedTo> {
        ...
        let mut cestory = {
            ...
            if cestory.args.no_rcu || benchmark::syncing() {
                // If RCU way is not suitable here, we do the traditional locked dispatch.
                return cestory.dispatch_blocks(self.req_id, blocks);
            }
    ...
    async fn init_runtime(
        ...
        self.lock_ceseal(false, false)?.init_runtime(
    ...
    async fn refresh_endpoint_signing_time(&mut self, _: ()) -> RpcResult<pb::GetEndpointResponse> {
        self.lock_ceseal(false, false)?.sign_endpoints()
    }

    async fn get_endpoint_info(&mut self, _: ()) -> RpcResult<pb::GetEndpointResponse> {
        self.lock_ceseal(true, false)?.get_endpoint_info()
    }

    async fn sign_endpoint_info(
        &mut self,
        request: pb::SignEndpointsRequest,
    ) -> Result<pb::GetEndpointResponse, crpc::server::Error> {
        self.lock_ceseal(true, false)?
    ...
    async fn handover_start(
        ...
        if !dev_mode && in_sgx {
            ...
            let my_runtime_timestamp = runtime_state
                .chain_storage
                .get_ceseal_added_at(&my_runtime_hash)
    ...
    ) -> Result<(), crpc::server::Error> {
        self.lock_ceseal(false, false)?.set_netconfig(request);
        Ok(())
    }
    ...
    ) -> Result<pb::HttpResponse, crpc::server::Error> {
        use reqwest::{
    ...
    async fn get_network_config(
        &mut self,
        _request: (),
    ) -> Result<pb::NetworkConfigResponse, crpc::server::Error> {
    ...
    async fn stop(&mut self, request: pb::StopOptions) -> Result<(), crpc::server::Error> {
        self.lock_ceseal(true, true)?
            .stop(request.remove_checkpoints)
    }
    async fn load_storage_proof(
        &mut self,
        req: cestory_api::crpc::StorageProof,
    ) -> Result<(), crpc::server::Error> {
        self.lock_ceseal(false, true)?
            .load_storage_proof(req.proof)?;
        Ok(())
    }
    async fn take_checkpoint(&mut self, _req: ()) -> Result<pb::SyncedTo, crpc::server::Error> {
        let synced_to = self
