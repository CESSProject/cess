...
impl TestNetFactory for RRSCTestNet {
	...
	fn peers_mut(&mut self) -> &mut Vec<RRSCPeer> {
		trace!(target: "rrsc", "Retrieving peers, mutable");
		&mut self.peers
	}
...
fn claim_epoch_slots() {
	...
	let mut epoch: Epoch = cessp_consensus_rrsc::Epoch {
		start_slot: 0.into(),
...
fn claim_vrf_check() {
	...
	let epoch: Epoch = cessp_consensus_rrsc::Epoch {
		start_slot: 0.into(),
