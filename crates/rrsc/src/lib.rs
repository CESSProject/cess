...
pub use cessp_consensus_rrsc::{
	...
	RRSCConfiguration, RRSCEpochConfiguration, ConsensusLog, Randomness, RRSC_ENGINE_ID,
};
...
const AUTHORING_SCORE_VRF_CONTEXT: &[u8] = b"substrate-rrsc-vrf";
	...
	C::Api: RRSCApi<B>,
{
	...
	let version = runtime_api.api_version::<dyn RRSCApi<B>>(at_hash)?;
...
) -> Result<RRSCWorker<B>, ConsensusError>
where
	B: BlockT,
	...
	Error: std::error::Error + Send + From<ConsensusError> + From<I::Error> + 'static,
{
	...
	Ok(RRSCWorker { inner: Box::pin(slot_worker), slot_notification_sinks })
}
	...
	C: HeaderBackend<B> + HeaderMetadata<B, Error = ClientError>,
{
	while let Some(request) = request_rx.next().await {
		match request {
			RRSCRequest::EpochData(response) => {
				let _ = response.send(epoch_changes.shared_data().clone());
			},
			RRSCRequest::EpochDataForChildOf(parent_hash, parent_number, slot, response) => {
				let lookup = || {
...
impl<B: BlockT> RRSCWorkerHandle<B> {
	async fn send_request(&self, request: RRSCRequest<B>) -> Result<(), Error<B>> {
	...
	pub async fn epoch_data(&self) -> Result<EpochChangesFor<B, Epoch>, Error<B>> {
		...
		self.send_request(RRSCRequest::EpochData(tx)).await?;
	...
	pub async fn epoch_data_for_child_of(
		...
		let (tx, rx) = oneshot::channel();
		self.send_request(RRSCRequest::EpochDataForChildOf(parent_hash, parent_number, slot, tx))
			.await?;
...
pub struct ImportQueueParams<'a, Block: BlockT, BI, Client, CIDP, SelectChain, Spawn> {
	/// The RRSC link that is created by [`block_import`].
	pub link: RRSCLink<Block>,
...
) -> ClientResult<(DefaultImportQueue<Block>, RRSCWorkerHandle<Block>)>
where
	...
	Spawn: SpawnEssentialNamed,
{
	...
	spawner.spawn_essential("rrsc-worker", Some("rrsc"), answer_requests.boxed());

	Ok((
		BasicQueue::new(verifier, Box::new(block_import), justification_import, spawner, registry),
		RRSCWorkerHandle(worker_tx),
	))
}
