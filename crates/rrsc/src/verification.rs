...
use crate::{
	...
	rrsc_err, find_pre_digest, BlockT, Epoch, Error, AUTHORING_SCORE_LENGTH,
...
fn check_primary_header<B: BlockT + Sized>(
	...
	if !AuthorityPair::verify(&signature, pre_hash, authority_id) {
		return Err(rrsc_err(Error::BadSignature(pre_hash)))
	}
	...
	if !authority_id.as_inner_ref().vrf_verify(&data, &pre_digest.vrf_signature) {
		return Err(rrsc_err(Error::VrfVerificationFailed))
	}
	...
	let score = authority_id
		...
		.map_err(|_| rrsc_err(Error::VrfVerificationFailed))?;

	if score >= threshold {
		return Err(rrsc_err(Error::VrfThresholdExceeded(threshold)))
	}
