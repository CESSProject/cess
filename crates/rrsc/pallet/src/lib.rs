...
use cessp_consensus_rrsc::{
	...
	EquivocationProof, Randomness as RRSCRandomness, Slot, RRSC_ENGINE_ID, RANDOMNESS_LENGTH,
...
pub mod pallet {
	...
	pub(super) type UnderConstruction<T: Config> = StorageMap<
		...
		Twox64Concat,
		u32,
		BoundedVec<RRSCRandomness, ConstU32<UNDER_CONSTRUCTION_SEGMENT_LENGTH>>,
		ValueQuery,
	>;
		...
		fn on_finalize(_now: BlockNumberFor<T>) {
			...
			if let Some(pre_digest) = Initialized::<T>::take().flatten() {
				...
				if let Some(signature) = pre_digest.vrf_signature() {
					let randomness: Option<RRSCRandomness> = Authorities::<T>::get()
						.get(authority_index as usize)
						.and_then(|(authority, _)| {
							let public = authority.as_inner_ref();
							let transcript = cessp_consensus_rrsc::make_vrf_transcript(
								&Self::randomness(),
								CurrentSlot::<T>::get(),
								EpochIndex::<T>::get(),
							);
...
impl<T: Config> Pallet<T> {
	...
	fn deposit_randomness(randomness: &RRSCRandomness) {
...
fn compute_randomness(
	last_epoch_randomness: RRSCRandomness,
	epoch_index: u64,
	rho: impl Iterator<Item = RRSCRandomness>,
