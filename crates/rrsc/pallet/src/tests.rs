...
use cessp_consensus_rrsc::{
	AllowedSlots, RRSCEpochConfiguration, Slot, VrfSignature, RANDOMNESS_LENGTH,
};
...
where
	F: Fn(cessp_consensus_rrsc::AuthorityIndex, Slot, VrfSignature) -> sp_runtime::Digest,
{
	let (pairs, mut ext) = new_test_ext_with_pairs(1);
...
fn report_equivocation_after_skipped_epochs_works() {
	...
	ext.execute_with(|| {
		...
		assert!(RRSC::report_equivocation_unsigned(
			RuntimeOrigin::none(),
			Box::new(equivocation_proof),
			key_owner_proof
		)
...
fn skipping_over_epochs_works() {
	...
	ext.execute_with(|| {
		...
		assert_eq!(RRSC::session_index_for_epoch(0), 0);
		assert_eq!(RRSC::session_index_for_epoch(1), 1);

		// otherwise the session index is offset by the number of skipped epochs
		assert_eq!(RRSC::session_index_for_epoch(4), 2);
		assert_eq!(RRSC::session_index_for_epoch(5), 3);
	});
}
