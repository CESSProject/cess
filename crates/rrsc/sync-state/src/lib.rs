...
use cessc_consensus_rrsc::{RRSCWorkerHandle, Error as RRSCError};
...
pub enum Error<Block: BlockT> {
	...
	#[error("Failed to load the RRSC epoch data: {0}")]
	LoadingEpochDataFailed(RRSCError<Block>),
...
pub struct SyncState<Block: BlockT, Client> {
	...
	rrsc_worker_handle: RRSCWorkerHandle<Block>,
}
	...
	Client: HeaderBackend<Block> + sc_client_api::AuxStore + 'static,
{
	...
	pub fn new(
		...
		if sc_chain_spec::get_extension::<LightSyncStateExtension>(chain_spec.extensions())
			.is_some()
		{
			Ok(Self { chain_spec, client, shared_authority_set, rrsc_worker_handle })
		} else {
	...
	async fn build_sync_state(&self) -> Result<LightSyncState<Block>, Error<Block>> {
		...
		Ok(LightSyncState {
			...
			rrsc_epoch_changes: epoch_changes,
