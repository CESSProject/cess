use ces_mq::traits::MessageChannel;
...
fn test_send_message() {
    ...
    use ces_mq::{MessageSendQueue, MessageSigner};
...

    {
        ...
        handle00.push_data(b"payload00".to_vec(), b"cess.network/test0".to_vec());

        let signer = TestSigner(b"key1".to_vec());

        let handle01 = queue.channel(runtime.clone(), signer);
        handle01.push_data(b"payload01".to_vec(), b"cess.network/test1".to_vec());

        handle00.push_data(b"payload02".to_vec(), b"cess.network/test1".to_vec());
...
#[test]
fn test_dispatcher() {
    use ces_mq::{Message, MessageDispatcher};
