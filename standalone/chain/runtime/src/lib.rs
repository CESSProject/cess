...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: Cow::Borrowed("cess-node"),
	impl_name: Cow::Borrowed("cess-node"),
	authoring_version: 1,
...
impl pallet_asset_conversion_tx_payment::Config for Runtime {
	...
	type OnChargeAssetTransaction =
		SwapAssetAdapter<Native, NativeAndAssets, AssetConversion, ResolveAssetTo<TreasuryAccount, NativeAndAssets>>;
...
pub fn dummy_babe_id_from_account_id(a: AccountId) -> BabeId {
	use polkadot_sdk::sp_core::ByteArray;
	let mut id = BabeId::from_slice(a.as_slice()).expect("dummy babe id");
	let id_raw: &mut [u8] = id.as_mut();
	id_raw[0..].copy_from_slice(a.as_ref());
	id_raw[0..4].copy_from_slice(&sp_core::crypto::key_types::BABE.0);
	id
}
...
fn transform_session_keys(v: AccountId, old: OldSessionKeys) -> SessionKeys {
	SessionKeys {
		...
		babe2: dummy_babe_id_from_account_id(v),
	}
}
...
impl pallet_cess_staking::Config for Runtime {
	...
	type DisablingStrategy = pallet_cess_staking::UpToLimitDisablingStrategy;
...
impl pallet_asset_conversion::Config for Runtime {
	...
	type PoolLocator = Chain<
		...
		>,
		Ascending<AccountId, NativeOrWithId<u32>, AccountIdConverter<AssetConversionPalletId, Self::PoolId>>,
	>;
...
mod runtime {
	...
	pub type BabeX = pallet_babe::Pallet<Runtime>;
	...
	pub type Staking = pallet_cess_staking::Pallet<Runtime>;
	...
	pub type CessTreasury = pallet_cess_treasury::Pallet<Runtime>;
	...
	pub type FileBank = pallet_file_bank::Pallet<Runtime>;

	#[runtime::pallet_index(105)]
	pub type Sminer = pallet_sminer::Pallet<Runtime>;

	#[runtime::pallet_index(106)]
	pub type Audit = pallet_audit::Pallet<Runtime>;

	#[runtime::pallet_index(107)]
	pub type StorageHandler = pallet_storage_handler::Pallet<Runtime>;

	#[runtime::pallet_index(108)]
	pub type Oss = pallet_oss::Pallet<Runtime>;

	#[runtime::pallet_index(109)]
	pub type Cacher = pallet_cacher::Pallet<Runtime>;

	#[runtime::pallet_index(110)]
	pub type Reservoir = pallet_reservoir::Pallet<Runtime>;
...
pub type CheckedExtrinsic = fp_self_contained::CheckedExtrinsic<AccountId, RuntimeCall, TxExtension, H160>;
...
impl pallet_asset_tx_payment::BenchmarkHelperTrait<AccountId, u32, u32> for AssetTxHelper {
	fn create_asset_id_parameter(_id: u32) -> (u32, u32) {
		unimplemented!("FIXME: to implement create_asset_id_parameter()");
	}
	fn setup_balances_and_pool(_asset_id: u32, _account: AccountId) {
		unimplemented!("FIXME: to implement setup_balances_and_pool()");
	}
}
...
impl_runtime_apis! {
	...
	impl sp_consensus_babe::BabeApi<Block> for Runtime {
		fn configuration() -> sp_consensus_babe::BabeConfiguration {
			let epoch_config = BabeX::epoch_config().unwrap_or(BABE_GENESIS_EPOCH_CONFIG);
			sp_consensus_babe::BabeConfiguration {
				slot_duration: BabeX::slot_duration(),
				epoch_length: EpochDuration::get(),
				c: epoch_config.c,
				authorities: BabeX::authorities().to_vec(),
				randomness: BabeX::randomness(),
				allowed_slots: epoch_config.allowed_slots,
			}
		}

		fn current_epoch_start() -> sp_consensus_babe::Slot {
			BabeX::current_epoch_start()
		}
		...
		fn submit_report_equivocation_unsigned_extrinsic(
			...
			BabeX::submit_unsigned_equivocation_report(
				equivocation_proof,
				key_owner_proof,
			)
		}
