...
pub struct EthDeps<B: BlockT, C, P, A: ChainApi, CT, CIDP> {
	...
	pub pubsub_notification_sinks: Arc<fc_mapping_sync::EthereumBlockNotificationSinks<fc_mapping_sync::EthereumBlockNotification<B>>>
}
	...
	EC: EthConfig<B, C>,
{
	...
	io.merge(
		Eth::<B, C, P, CT, BE, A, CIDP, EC>::new(
			...
			forced_parent_hashes,
			pending_create_inherent_data_providers,
			None, // Some(Box::new(AuraConsensusDataProvider::new(client.clone()))),
		)
		.replace_config::<EC>()
		.into_rpc(),
	)?;
