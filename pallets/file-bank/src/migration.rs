...
impl<T: Config, const TEST_ALL_STEPS: bool> OnRuntimeUpgrade for Migration<T, TEST_ALL_STEPS> {
	...
	fn post_upgrade(_state: Vec<u8>) -> Result<(), TryRuntimeError> {
		...
		if let Some(hash) = crate::File::<T>::iter_keys().next() {
			crate::File::<T>::get(hash).expect("CodeInfo exists for hash; qed");
		}
		if let Some(hash) = crate::DealMap::<T>::iter_keys().next() {
			crate::DealMap::<T>::get(hash).expect("PristineCode exists for hash; qed");
		}
...
impl<T: Config, const TEST_ALL_STEPS: bool> Migration<T, TEST_ALL_STEPS> {
	...
	pub(crate) fn migrate(mut meter: &mut WeightMeter) -> MigrateResult {
		...
		if meter.try_consume(T::WeightInfo::migration_noop()).is_err() {
