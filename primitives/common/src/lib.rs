...
use codec::{EncodeLike, MaxEncodedLen, Output, Input, Error};
use scale_info::TypeInfo;

#[derive(Copy, Clone, Eq, PartialEq, RuntimeDebug, MaxEncodedLen)]
...
impl Hash {
	fn slice_to_array_68<T>(slice: &[T]) -> Result<&[T; 68], TryFromSliceError> {
		if slice.len() == 68 {
			let ptr = slice.as_ptr() as *const [T; 68];
			unsafe { Ok(&*ptr) }
		} else {
	...
	pub fn from_shard_id(shard_id: &[u8; 72]) -> Result<Self, TryFromSliceError> {
		let slice = Self::slice_to_array_68::<u8>(&shard_id[0..67])?;
		let hash = Hash(*slice);
		return Ok(hash)
	}
}
